...
static void handle_client(client_t *c){
  ...
  char *buf=calloc(1,MAX_REQ+1); if(!buf){ close(fd); free(c); return; }
  size_t used=0; while(used<MAX_HDR){ ssize_t n=recv(fd,buf+used,MAX_REQ-used,0); if(n<=0){ free(buf); close(fd); free(c); return; } used+=(size_t)n; buf[used]='\0'; if(strstr(buf,"\r\n\r\n")) break; }
  ...
  if((size_t)req.content_length>MAX_REQ-h_end){ error_resp(fd,413,"Payload Too Large"); free(buf); close(fd); free(c); return; }
  size_t body_have=used>h_end?used-h_end:0; while(body_have<(size_t)req.content_length){ ssize_t n=recv(fd,buf+used,MAX_REQ-used,0); if(n<=0){ free(buf); close(fd); free(c); return; } used+=(size_t)n; body_have+=(size_t)n; }
  int bfd=connect_backend(c->backend,c->backend_port,c->timeout); if(bfd<0){ error_resp(fd,502,"Bad Gateway"); log_line(c->ip,req.method,req.target,502); free(buf); close(fd); free(c); return; }
  char fwd[MAX_HDR+1024]; size_t fwd_len=0; if(http_build_forward_request(&req,c->ip,fwd,sizeof(fwd),&fwd_len)){ error_resp(fd,500,"Internal Server Error"); close(bfd); free(buf); close(fd); free(c); return; }
  if(send_all(bfd,fwd,fwd_len)|| (req.content_length>0 && send_all(bfd,buf+h_end,(size_t)req.content_length))){ error_resp(fd,502,"Bad Gateway"); close(bfd); free(buf); close(fd); free(c); return; }
  ...
  log_line(c->ip,req.method,req.target,status); close(bfd); free(buf); close(fd); free(c);
}
...
int main(int argc,char **argv){
  signal(SIGPIPE,SIG_IGN); int listen_port=8080,backend_port=9000,timeout=10,rate=60,window=10,workers=DEFAULT_WORKERS; char backend[256]="127.0.0.1";
  for(int i=1;i<argc;i++){ if(i+1>=argc){fprintf(stderr,"bad args\n");return 2;} char *k=argv[i],*v=argv[++i]; if(!strcmp(k,"--listen")) listen_port=atoi(v); else if(!strcmp(k,"--backend-host")) snprintf(backend,sizeof(backend),"%s",v); else if(!strcmp(k,"--backend-port")) backend_port=atoi(v); else if(!strcmp(k,"--timeout")) timeout=atoi(v); else if(!strcmp(k,"--rate-limit")) rate=atoi(v); else if(!strcmp(k,"--rate-window")) window=atoi(v); else if(!strcmp(k,"--workers")) workers=atoi(v); else {fprintf(stderr,"unknown option: %s\n",k);return 2;} }
  if(listen_port<1||listen_port>65535||backend_port<1||backend_port>65535||timeout<1||rate<1||window<1||workers<1||workers>128){fprintf(stderr,"invalid config\n");return 2;}
  ...
  printf("CinderProxy listening on :%d -> %s:%d workers=%d queue=%d\n",listen_port,backend,backend_port,workers,QUEUE_CAPACITY);
  for(;;){
    ...
    c->fd=cfd;c->backend_port=backend_port;c->timeout=timeout;c->limiter=&lim;snprintf(c->backend,sizeof(c->backend),"%s",backend);
    ...
    if(!queue_push(c)){
      error_resp(cfd,503,"Service Unavailable");
      close(cfd);
      free(c);
    }
  }
